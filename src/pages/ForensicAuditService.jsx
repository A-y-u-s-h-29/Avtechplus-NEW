    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl md:text-5xl font-black text-gray-900 uppercase leading-tight">
          Forensic Audit for <span className="text-orange-500">Projects</span>
        </h2>
      </div>

      <p className="text-gray-700 leading-relaxed">
        A Forensic Audit for Projects in India is a specialized audit aimed at detecting fraud, misappropriation, corruption, cost overruns, contractual violations, and inefficiencies in large capital-intensive projects like roads, railways, power plants, warehouses, and EPC works. <br />Forensic Audit is highly relevant— especially for projects involving capex, vendors, logistics contracts, and publicprivate partnerships (PPP).
      </p>
      ...
      <div className="text-gray-700 leading-relaxed space-y-2">
        ...
        <ul className="list-disc pl-5 space-y-1">
          <li className="font-normal">Identifying frauds or leakages</li>
          <li className="font-normal">Examining contract compliance</li>
          <li className="font-normal">Tracing fund flow</li>
        </ul>
        <p className="text-xs text-gray-400 pt-1">AVtech/Forensic Audit/Page 2 of 6</p>
        <ul className="list-disc pl-5 space-y-1">
          <li className="font-normal">Project Execution Audit</li>
          <li className="font-normal">Supply chain assessment</li>
          <li className="font-normal">Business risk assessment</li>
        </ul>
      </div>
    </div>
      ...
      {auditPoints.map((point, index) => (
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row items-stretch w-full">
            ...
            {/* LEFT half: All text content on the left */}
            <div className="flex-1 w-full flex flex-col justify-center p-6 md:p-8 lg:p-10">
              <div className="flex flex-col max-w-full w-full">
                
                {/* Title section */}
                <div className="mb-4">
                  <h4 className="font-bold text-gray-800 text-xl md:text-2xl uppercase tracking-wide text-left">
                    {point.title}
                  </h4>
                  {point.subtitle && (
                    <span className="text-xs text-gray-400 mt-1 block text-left font-normal">{point.subtitle}</span>
                  )}
                </div>

                {/* General details listed vertically */}
                {point.details && point.details.length > 0 && (
                  <div className="space-y-2 w-full mb-4 border-b border-gray-100 pb-4">
                    {point.details.map((detail, idx) => (
                      <div key={idx} className="text-sm md:text-base text-gray-600 text-left font-normal">
                        • {detail}
                      </div>
                    ))}
                  </div>
                ...
                {/* Red flag text lists on the left - now normal weight */}
                {point.redFlags && point.redFlags.length > 0 && (
                  <div className="space-y-3 w-full">
                    <span className="text-xl font-bold uppercase tracking-wider text-red-500 block text-left">
                      Some Red Flags:
                    </span>
                    {point.redFlags.map((flag, idx) => (
                      <div key={idx} className="flex items-start gap-2">
                        <div className="w-2 h-2 rounded-full bg-red-500 mt-2 flex-shrink-0"></div>
                        <span className="text-sm md:text-base text-gray-700 leading-relaxed text-left font-normal">
                          {flag}
                        </span>
                      </div>
                    ))}
            ...
            {/* RIGHT half: Same size image for all points */}
            <div className="flex-1 w-full flex justify-center items-center p-6 md:p-8 lg:p-10 bg-gray-50/50 lg:bg-transparent">
              <div className="w-full max-w-[280px] md:max-w-[320px] lg:max-w-[350px]">
                <img
                  src={point.image}
                  alt={point.title}
